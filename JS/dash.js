...
function fetchData(uid) {
    ...
    firebase.database().ref('Users/' + uid).on('value', (snapshot) => {
        ...
        firebase.database().ref('Data/' + uid + "/" + today).on('value', (snapshot1) => {
            var steps = 0;
            if (snapshot1.hasChild('steps')) {
                steps = snapshot1.val().steps
            }
            firebase.database().ref('Goals/' + uid).on('value', (snapshot2) => {
                var stepGoal = 500;
                var calorieGoal = 100;
                if(snapshot2.child('stepGoal').exists()){
                    stepGoal = snapshot2.val().stepGoal
                    calorieGoal = snapshot2.val().caloriesGoal
                }
