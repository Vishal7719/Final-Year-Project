...
<div id="mainContainerDiv" style="background-image: url('../img/bg1.jpg'); background-repeat:no-repeat;">
        ...
        firebase.database().ref('FAQ').on('value', (snapshot) => {
            try {
                snapshot.forEach(s => {
                    // console.log(s)
                    const data = s.val();
                    document.getElementById('content').innerHTML += '<div class="card my-3"  margin: 20px;">\
